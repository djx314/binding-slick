...
trait Tables {
  ...
  class Companies(_tableTag: Tag) extends Table[CompaniesRow](_tableTag, "COMPANIES") {
    ...
    def ? =
      (Rep.Some(id), Rep.Some(name)).shaped.<>({ r =>
        import r._; _1.map(_ => CompaniesRow.tupled((_1.get, _2.get)))
  ...
  class Users(_tableTag: Tag) extends Table[UsersRow](_tableTag, "USERS") {
    ...
    /** Maps whole row to an option. Useful for outer joins. */
    def ? =
      (Rep.Some(id), Rep.Some(name), companyId).shaped.<>({ r =>
    ...
    lazy val companiesFk =
      foreignKey("IDX_USERS_FK0", companyId, Companies)(r => Rep.Some(r.id), onUpdate = ForeignKeyAction.Restrict, onDelete = ForeignKeyAction.Restrict)
  }
