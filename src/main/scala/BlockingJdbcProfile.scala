...
trait Test {
  ...
  sql"""abc""".query[String].unique.transact(xa)

  val aa: JdbcBackend#Database = ???
...
trait BlockingJdbcProfile extends JdbcProfile with BlockingRelationalProfile {
  ...
  trait BlockingAPI extends super.BlockingAPI with ImplicitColumnTypes with slick.JdbcProfileBlockingSession {
    ...
    private class BlockingJdbcActionContext(s: JdbcBackend#Session) extends backend.JdbcActionContext {
      override val useSameThread          = true
    ...
    class QueryInvoker[R](tree: Node, param: Any) extends QueryInvokerImpl[R](tree, param, null) {
      def selectStatement: String = getStatement
      def rowReader =
        new Read[R](gets = List.fill(converter.width)((null, null)), unsafeGet = { (rs, index) =>
          converter.read(rs)
        })

      def doobieQuery: Query0[R] =
        Fragment
          .apply(getStatement, ())(new Write[Unit](puts = List.empty, toList = { _: Unit =>
            List.empty
          }, unsafeSet = { (ps, index, unit) =>
            setParam(ps)
          }, unsafeUpdate = { (rs, index, unit) =>
            ()
          }))
          .query(rowReader)
    }
    ...
    class BlockingInsertInvoker[U](compiled: CompiledInsert) {
      ...
      def doobieInsert: Update[U] = {
        Update(compiled.standardInsert.sql)(
          new Write[U](
              puts = List.fill(compiled.standardInsert.fields.length)((null, null))
            , toList = { _: U =>
              List.fill(compiled.standardInsert.fields.length)(null)
            }
            , unsafeSet = { (ps, index, data) =>
              compiled.standardInsert.converter.set(data, ps)
            }
            , unsafeUpdate = { (rs, index, data) =>
              compiled.standardInsert.converter.update(data, rs)
            }
          ))
      }
    ...
    implicit def compiledToInsertInvoker[U, C[_]](c: RunnableCompiled[_ <: Query[_, _, C], C[U]]) =
